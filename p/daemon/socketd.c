// SilberLand MUDlib
//
// p/daemon/socketd.c -- socket daemon
//
// $Date: 1997/11/09 19:18:33 $
/* $Revision: 1.1 $
 * $Log: socketd.c,v $
 * Revision 1.1  1997/11/09 19:18:33  Woody
 * Initial revision
 *
 */
...
#if 1
#define debug(X) if (find_player("querolin")) \
		 tell_object(find_player("querolin"), X)
#endif
...
int socket_write(int fd, mixed mesg)
{
    ...
    size=(stringp(mesg)) ? sizeof(mesg) : sizeof(mesg);
...
int valid_socket(object ob)
{
    return (object_name(ob)[0..7]=="/secure/") ||
           (object_name(ob)[0..9]=="/p/daemon/");
}
