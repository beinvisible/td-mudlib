...
string _query_color(object ob)
{
  string s1;
  if (!interactive(ob)) return "";

  ob->Set(P_COLOR_COMMUNICATION, SAVE, F_MODE_AS);
  if ( (ob->QueryProp(P_COLOR_COMMUNICATION)==0) ||
       (ob->QueryProp(P_COLOR_COMMUNICATION)=="0") )
  {  return ""; }
     s1="";
  switch(to_string(ob->QueryProp(P_COLOR_COMMUNICATION))) {
     case "1": s1+=ANSI_BLACK; break; //schwarz
     case "2": s1+=ANSI_RED; break; //dunkelrot
     case "3": s1+=ANSI_GREEN; break; //gruen
     case "4": s1+=ANSI_BOLD+ANSI_YELLOW; break; //gelb
     case "5": s1+=ANSI_BLUE; break; //blau
     case "6": s1+=ANSI_PURPLE; break; //lila
     case "7": s1+=ANSI_CYAN; break; //hellcyan
     case "8": s1+=ANSI_WHITE; break; // weiss
     case "9": s1+=ANSI_BOLD; break; // fett (weiss)
     case "10": s1+=ANSI_BOLD+ANSI_BLUE; break; //hellblau
     case "11": s1+=ANSI_YELLOW; break;  //braun
     case "12": s1+=ANSI_BOLD+ANSI_BLACK; break; //grau
     case "13": s1+=ANSI_BOLD+ANSI_CYAN; break; //hellcyan
     case "14": s1+=ANSI_BOLD+ANSI_GREEN; break; //hellgruen
     case "15": s1+=ANSI_BOLD+ANSI_RED; break; //hellrot
     case "16": s1+=ANSI_BOLD+ANSI_PURPLE; break; //helllila
     default: s1=ANSI_NORMAL;
  }
  return s1;
}
...
private int _send(object ob, string message, int flag)
{
  ...
  color="";
  if (interactive(ob)) { color=_query_color(ob); }
...
static int ignoriere(string str)
{
  ...
  if (member(lower_case(str), ignore)!=-1)
  {
...
static int _communicate(mixed str, int silent)
{
  ...
  if (verb == "rede" && !IS_WIZARD(ME))
  {
    ...
    say(break_string(str, 78, capitalize(myname)+" sagt: "), taub+not_same);
    ...
    switch (capitalize(_my_race))
    {
      case "Zwerg":
        ...
        break;
      case "Elf":
        say(capitalize(myname)+" singt leise vor sich hin.\n", taub+same);
        break;
      case "Mensch":
  ...
  if(!silent) write(break_string(str, 78, "Du sagst: "));
  return 1;
}
...
static int _shout_to_all(mixed str)
{
  ...
  write(color+break_string(str, 78, "Du rufst: ")+ANSI_NORMAL);
...
varargs int _tell(mixed str,int intcall)
{
  ...
  if (visflag)
    write(color+break_string(msg, 78, "Du teilst "+capitalize(it)+" mit: ")
               +ANSI_NORMAL);
...
static int _whisper(string str)
{
  ...
  myname = capitalize((((IS_WIZARD(ME) && !QueryProp(P_INVIS) &&
                         (QueryProp(P_CAN_FLAGS) & CAN_PRESAY))?
                        QueryProp(P_PRESAY) : "") + ME->name()) || "");
  _send(ob, sprintf(_query_color(ob)+"%s flüstert Dir zu: %s\n"+ANSI_NORMAL,
...
int erwidere(string str)
{
  ...
  _notify_fail(_query_color(this_player())+
               "Du hast aber noch keine Mitteilungen erhalten, "
