...
mixed valid_read(string path, string euid, string fun, object obj) {
  ...
  if(sizeof(strs) > 1 && strs[0] == "doc") 
  { return path; }
  if(sizeof(strs) > 1 && strs[0] == "open")
  { return path; }

  /* alle < 20 abblocken! */
  if(find_living("samtpfote"))
  {
  	tell_object(find_living("samtpfote"),
  		sprintf("%O: euid=%s\n", 
  		previous_object(), euid));
  }
  //if(!IS_WIZARD(euid)) return 0;
 
  if(sizeof(strs)>1 && strs[0]=="wahl") return 0;
  if(sizeof(strs)>1 && strs[0]=="news" &&
    (strs[<1][0..8]=="erzmagier" || strs[<1][0..12]=="OLD.erzmagier") &&
    !IS_ARCH(euid)) return 0;
  if(sizeof(strs)>1 && strs[0]=="news" &&
    (strs[<1][0..12]=="regionsmagier" || strs[<1][0..16]=="OLD.regionsmagier") &&
    !IS_LORD(euid)) return 0;
  ...
  {
  	/* grundsaetzlich koennen NUR domainmember da zugreifen */
  	if(!IS_DOMAINMEMBER(euid)) return 0;  
  	
  	/* RM koennen alles in /d/ lesen */
  	if(IS_LORD(euid)) return path;
  	
  	/* Mitarbeiter in dieser Region? */

	/*    regionshome */  	
	if(sizeof(strs) >= 3 && strs[3] == euid)
...
int access_rights(string *p_arr, string euid)
{
  ...
  i=sizeof(p_arr)-2;
  while (i>=0 && file_size(implode(p_arr[0..i],"/")+"/access_rights.c")<0) i--;
  if (i<0) return 0;
...
mixed valid_write(string path, string euid, string fun, object obj) {
  ...
  if(s>1) {
	...
	break;
      ...
      // if (s==2) return domain_master(euid,strs[1]);  // Eher unnoetig...
      if (strs[2]==euid || 
