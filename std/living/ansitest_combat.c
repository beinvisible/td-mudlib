 ...
 * Und naochmal: Trainigsraum fuer Karateka.
 *
...
int Kill(object ob)
{
  ...
  if (query_once_interactive(ob) && query_once_interactive(ME) &&
      !IsEnemy(ob))
  {
    (void)QueryPlAttacked(ME);
    (void)ob->QueryPlAttacked(ob); // aktualisieren ...
  }
  res=InsertEnemy(ob);
  ...
  arena=0;
  /*
  arena=((environment(ME)==find_object("/d/wueste/room/arena/arena"))||
...
void StopHuntText(object arg)
{
  ...
  tell_object(arg, ME->Name(WER)+" jagt Dich nicht mehr.\n");
  tell_object(ME,"Du jagst "+arg->name(WEN)+" nicht mehr.\n");
}
...
void Attack(object enemy)
{
  ...
  if (damage_msg2!=last_attack_msg)
  {
    ...
    if (!(enemy->QueryProp(P_BRIEF) & BRIEF_FNOATTACK))
      tell_object( enemy, "  "+ME->Name(WER,1) + " greift Dich" +
                   damage_msg2 + " an.\n" );
  }
