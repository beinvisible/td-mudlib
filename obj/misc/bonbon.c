...
void geschmack()
{
	...
	switch(num)
	{
		case 0 :	
			...
			SetProp(P_LONG,"Ein gefülltes Bonbon mit Orangengeschmack.\n");
			SetProp(P_NAME,"Orangenbonbon");
			AddId(({"orangenbonbon"}));
			break;
		case 1 :
			SetProp(P_SHORT,"Ein Zitronenbonbon");
			SetProp(P_LONG,"Ein gefülltes Bonbon mit Zitronengeschmack.\n");
			...
			SetProp(P_LONG,"Sehr süß und sehr klebrig.\n");
		...
		case 7 :
			...
			SetProp(P_LONG,"Dieses himbeerige Fruchtbonbon ist viel größer als eine Himbeere.\n");
			SetProp(P_NAME,"Himbeerdrops");
			AddId(({"drops","himbeerdrops","fruchtbonbon"}));
			break;
                default :
			SetProp(P_SHORT,"Ein Multivitaminbonbon");
			SetProp(P_LONG,"Ein Multivitaminbonbon, sehr gesund (besonders bei dem Wetter in Münster).\n");
...
int essen(string str)
{
	...
	if ( !str || !id(str) )
	    return 0;
	write("Du lutschst genüsslich " + name(WEN,0) + ".\n");
