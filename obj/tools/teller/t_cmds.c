...
static scan_obj( object player, object obj )
{
  ...
  printf( "Nahrung....: [%4d/%4d] Flüssigk.: [%4d/%4d] " +
    "Alkohol........: [%4d/%4d].\n",
  ...
  switch( obj->QueryProp(P_GENDER) )
  {
    ...
    case MALE   : gender = "männlich  " ; break ;
  ...
  printf(
	...
	printf( "Geld.......: %11d Trägt....: %11d Playerkills....: %11d.\n",
  ...
  // 8.Zeile : Ruestung(en)[ WC ]
	...
	else
	{
		...
		weapon = sprintf( "kämpft%s [%d]", hands[0], hands[1] );
	}
	ac = 0;
...
static int cmd_secureinv()
{
	secureinv = !secureinv;
	if( secureinv )
		memo( "Inventory wird überwacht" );
	else
		memo( "Inventory wird nicht überwacht" );
