  ...
  for (i=sizeof(info)-1;i>=0;i--)
  {
    ...
    */
    if (info[i][D_FLAGS] & DOOR_RESET_CL)
    {
      ...
      if (door_status[dkey]>0)
      {
        door_message(env,info[i][D_NAME],info[i][D_GENDER]," schließt sich.");
        door_message_other(source,dest," schließt sich.");
      }
    ...
    if (info[i][D_FLAGS] & DOOR_RESET_OP)
    {
      ...
      if (door_status[dkey]<=0)
      {
        door_message(env,info[i][D_NAME],info[i][D_GENDER]," öffnet sich.");
        door_message_other(source,dest," öffnet sich.");
      }
      door_status[dkey]=1;
    }
  }
...
varargs int go_door (string str)
{
  ...
  {
    if (!mappingp(info[i])) continue;
    if (member(str,info[i][D_CMDS])<0) continue;
    dest=info[i][D_DEST];
    dkey=((string)((source<dest)?(source+":"+dest):(dest+":"+source)));
...
int oeffnen (string str)
{
  ...
  */
  ...
  {
    if (!mappingp(info[i])) continue;
    if (member(s1,info[i][D_IDS])<0) continue; // Falsche Tuer
    ob->SetProp(P_NAME,info[i][D_NAME]);
    ob->SetProp(P_GENDER,info[i][D_GENDER]);
    dest=info[i][D_DEST];
    dkey=((string)((source<dest)?(source+":"+dest):(dest+":"+source)));
    ...
    init_doors();
    write("Du öffnest "+ob->name(WEN)+".\n");
    say(capitalize(ob->name(WER))+" wird von "+this_player()->name(WEM)+
        " geöffnet.\n");
    door_message_other(source,dest," wird von der anderen Seite geöffnet.");
    return 1;
  }
  return 0;
}
