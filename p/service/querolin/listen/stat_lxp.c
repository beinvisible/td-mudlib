/* Eventuell mal eine Lost-XP-Top-Liste
 * Querolin@Tamedhon 14.01.2001
 *
...
#define DUMPFILE	"/p/service/querolin/listen/lxpliste.sort"
#define ADB(x)          if(find_player("querolin")) \
                            tell_object(find_player("querolin"), \
                            break_string(x, 79, "{ abg } "));

int	*ratings, *levels, *xp, *qp, *lxp, *deads;
string	*spieler, *flags, *guilds, *races;

/* --- Hilfsfunktionen ------------------------------------------------- */
void save_me() { save_object("/p/service/querolin/listen/lxpliste"); }
...
void clean_list()
{
    int		*rneu, *lneu, *xneu, *qneu, *lxpneu, *deadsneu; 
...
{
    int		hrating, hlevel, hxp, hqp, hdeads, hlxp;
...
//    ADB("dumping toplist ("+to_string(is)+" entries)");
    ...
    write_file(DUMPFILE, "\n-* Besuchsliste des Todes *-\n");
    write_file(DUMPFILE, "   Aktualisiert: "+dtime(time())+"\n");
    write_file(DUMPFILE,"\n"+
       "Rang   Name             Stufe    Tode  Verlorene Erfahrungspunkte\n\n");
    fmtstr="%3i.  %s%15-s    %3i  %4i    %"+strlen(to_string(lxp[0]))+"s\n";
...
//    ADB("trying to register '"+name+"'.");    
	...
	lxp+=({ pl->QueryProp(P_LOST_XP) });
        deads+=({ pl->QueryProp(P_DEADS) });	
...
//    	ADB("... updating "+name);
	...
	xp[p]=pl->QueryProp(P_XP); lxp[p]=pl->QueryProp(P_LOST_XP);
        ratings[p]=1; qp[p]=pl->QueryProp(P_QP);
        deads[p]=pl->QueryProp(P_DEADS);
	fl=FL_SPIELER;
	if(IS_LEARNER(pl)) { fl=FL_MAGIER; }
	else if(pl->QueryProp(P_TESTPLAYER)) { fl=FL_TESTIE; }
	else if(pl->QueryProp(P_SECOND)) { fl=FL_ZWEITIE; }
...
{
    seteuid(getuid());
    if(!restore_object("/p/service/querolin/listen/lxpliste"))
    {
	ratings=({}); levels=({}); xp=({}); flags=({}); spieler=({});
	qp=({}); guilds=({}); races=({}); lxp=({}); deads=({});
    }
//    ADB("geladen...");
}
