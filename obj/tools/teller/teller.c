	...
	if( getuid(PL) == getuid() )
	{
		answer +=
			( mit_say
				...
				? "Die Teller haben ein verschnörkeltes Muster.\n"
				: "Die Teller sind geradezu schmucklos.\n"
			) + ( dologaccess
				? "Die Teller spüren Zugriffe.\n"
				: ""
			) + ( secureinv
...
int jonglier( string str )
{
	...
	if( !str || id(str) || sscanf( str, "mit %s", arg ) == 1 )
	{
		if( !secure() )
			write(
				"Dieser Tellerstapel ist nichts für Dich. Suche Dir einen eigenen.\n"
			);
		else
			write(
...
int parse( string str )
{
	...
	if( mit_say && !PL->QueryProp(P_INVIS) )
	{
		if( !fehler_passiert )
			...
			say( PL->name(WEM)+" fällt beim Jonglieren ein Teller herunter.\n" );
	}
	if( !fehler_passiert )
		write( "OK.\n" );
	return TRUE;
