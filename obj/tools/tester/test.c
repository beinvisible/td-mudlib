...
** 
**         22.04.2025    Tamedhon Port durch Aloy
**
****************************************************************************
**
...
{   string re,PRE,END;
    ...
    re = break_string(
         "Mit diesem Objektprüfer kannst Du feststellen, welche Details, "+
         "Gerüche oder Geräusche in einem Raum bzw an einem Objekt noch "+
         ...
         "auf einmal testen lassen. Das dauert natürlich eine Weile. "+
         "Außerdem muss ein Verzeichnis '/players/<magiername>/' bestehen.",78)+
...
// Properties
    ...
    SetProp(P_SHORT,"Ein Objektprüfer") ;
    Set(P_LONG,#'x_long,F_QUERY_METHOD);
    SetProp(P_NAME,"Objektprüfer");
    ...
    AddId( ({"prüfer","objektprüfer", "test", "tester", "otest"}) );
...
int removen()
{
    write("Dieses Tool ist nur für Testies und Magier!\n");
    remove();
    return 1;
}
...
{
    return regexp(regexplode(s,"[^A-Za-z0-9ÄÖÜäöüß]"),"\\<[A-ZÄÖÜ]");
}
...
{   string *words,str,*re;
    ...
    words=regexplode(input,"[^A-Za-z0-9ÄÖÜäöüß]");
        n=sizeof(words);
    if (!n)
        return re;
    ...
    for (i=0;i<n;i++)
    {
        ...
        if (sizeof(regexp(({str}),"[^A-Za-z0-9ÄÖÜäöüß]")))
            continue;
...
{   string *strs,ostr,*strs2,suf;
    ...
    for ( strs2=({}),i=sizeof(strs)-1 ; i>=0 ; i--)
        ...
        {
            if (stringp(def) && all_dets[strs[i]]==def)
                continue;
            strs2 += ({ strs[i] });
	}
    if (sizeof(strs2)>0)
...
// ob P_INDOORS gesetzt ist oder nicht.
    ...
    if (room && r->QueryProp(P_INDOORS))
        zufinden = ({"Boden","Decke","Wand","Wände","Raum"});
    else if (room)
...
//  Details und SpecialDetails auswerten - gar nicht so einfach.
    ...
    all_dets = r->QueryProp(P_DETAILS);
...
//  Grossgeschriebene Woerter aus der Langbeschreibung hinzufuegen.
    ...
    if (function_exists("int_long",r))
        zufinden += det_auswerten(
            old_explode(r->QueryProp(P_INT_LONG),"Es gibt ")[0]);
    else
        zufinden += det_auswerten(old_explode(r->long(),"enthält:")[0]);
...
{   string re,PRE,END,WAS,*senses,s1,*zufinden,*s2,*s3,def,*uebergehen;;
    ...
    {
        all_dets=r->QueryProp(P_SMELLS)||([]);
        WAS="Gerüche";
    }
    else if (sense==SENSE_SOUND)
    {
        all_dets=r->QueryProp(P_SOUNDS)||([]);
        WAS="Geräusche";
    }
    else
    {
...
// Testen, welche Sense-Details fehlen und anzeigen
    ...
    if (scanrun<1)
    {
        ...
        {
            for ( s1="",--i ; i>=0 ; i-- )
                s1 += ((s2[i]=="default"?"DEFAULT":s2[i])+(i?"\n":""));
    ...
    {
        for ( s1="",--i ; i>=0 ; i-- )
           s1 += ((s3[i]=="default"?"DEFAULT":s3[i])+(i?"\n":""));
...
{   string *arg_array,dum;
    ...
    if (!objectp(targ))
    {
        write("OTEST-ERROR: Gewünschtes Ziel nicht gefunden.\n");
        return 0;
    }
...
{   string vz,af,u,*dir;
    ...
    if (scanrun)
    {
        write("Derzeit wird ein Verzeichnis gescannt. Bitte warten.\n");
        return 1;
    }

    if (object_name(ME)[0..2]=="/p/")
    {
        write(break_string("Ein Dirscan kann nur durchgeführt werden, "+
              "wenn man den Prüfer in sein /players/-Verzeichnis kopiert "+
...
// Zielfilename testen/festlegen.
    ...
    u=getuid(PL);
    if (file_size("/players/"+u+"/")!=-2)
    {
        printf("OTEST: Es existiert kein Verzeichnis '/players/%s/'\n",u);
        return 1;
    }
    for (i=sizeof(af)-1;i>=0;i--)
        if (((si=af[i])<48 && si!=46) || (si>57 && si<65) || si>122 ||
            (si>90 && si<97))
        {
            write("OTEST: Illegaler Filename für Ausgabefile.\n");
            return 1;
        }
    u=sprintf("/players/%s/%s",u,af);

// Zu untersuchendes Verzeichnis pruefen/einlesen.

    if (!vz || !stringp(vz) || sizeof(vz)<3 || vz[0]!='/' || vz[<1]!='/')
    {
        write("OTEST: Verzeichnisname muss mit '/' beginnen und aufhören.\n");
        return 1;
    }
    for (i=sizeof(af)-1;i>=0;i--)
        if (((si=af[i])<46) || (si>57 && si<65) || si>122 || (si>90 && si<97))
        {
            write("OTEST: Illegaler Filename für Verzeichnis.\n");
            return 1;
        }
    dir = get_dir(vz+"*.c");
    if ((si=sizeof(dir))<1)
    {
        write("OTEST: Das gewünschte Verzeichnis existiert nicht oder ist "+
              "leer.\n");
        return 1;
    }
