...
//
// 19-02-1996	20:00:00	Mupfel
// Lupe zeigt bei 'info' auch P_MATERIAL an, falls gesetzt
//
// 19 Oct 1997  Woody
// Mehr Security
//
// 11 Feb 1998  Feyaria  IS_WIZARD Abfrage
/* $Date: 2001/01/13 13:07:34 $
 * $Revision: 1.2 $
 * $Log: lupe.c,v $
 * Revision 1.2  2001/01/13 13:07:34  Samtpfote
 * Mehr Sicherheit, nun ab Magierlevel 25 benutzbar.
 *
 * Revision 1.1  2001/01/13 13:05:46  Samtpfote
 * Initial revision
 *
 */
...
string *commands, *functions, user, *query_list,logcmd;
mixed* argv;
...
{
  if (!owner)
    return destruct(this_object()), -5;
  if (objectp(dest) && dest != owner)
    return -5;
  if (stringp(dest) && owner != find_player(dest))
    return -5;
...
static int tell(string str)
{
  ...
  if (sizeof(tmp=explode(str," "))<2) return 0;
...
static int set_user(string str)
{
  string who;
  if (!str) return 0;
  if (str=="no access")
...
static string getarg(string args)
{
  string arg;
  string rest;
...
static string getrest(string str)
{
  if (arglen==0)
    return str;
...
static string stk(string arg);

static int interactiveMode(string str)
{
  if (str)
    return eval(str);
  stk("");
...
static int eval(string str)
{
  ...
  if(owner && !IS_ARCH(owner) && 
      !(logcmd == "here make" && explode(object_name(environment(owner)),"/")[0] == "players"))
  {
    write_file("/log/ARCH/lupe.log", ctime()+" ("+getuid(owner)+") "+
        logcmd+" (env: "+object_name(environment(owner))+ ")\n");
    write_file("/log/ARCH/tools/" + getuid(owner), 
        sprintf("%s: lupe in %s\n    %s\n", ctime(),
         object_name(environment(owner)), logcmd));
  }
  return 1;
}

static int push(object ob)
{
  stack+=({ob});
...
static string toggle_short(string arg)
{
...
static string pl(string arg)
{
  ...
  rest=getrest(arg);
  if (err) return 0;
  if (p=find_player(who) || p=find_netdead(who)) {
    push(p);
    return rest;
  }
...
static string me(string arg)
{
  push(this_player());
  return arg;
}

static string make_path(string path)
{
  return "secure/master"->_get_path( path, geteuid( this_player() ) );
}

static string desc(object ob);

static string new(string arg)
{
...
static string xpush(string arg)
{
  string rest,what;
  object ob;
...
static string ob(string arg)
{
  ...
  if (!ob)
  {
    ...
    if (!err)
      ob = find_object(file);
    if (!ob)    // Woody, 2002-01-25, don't know if OK this way ;)
      err = tmp;
  }
...
static string file(string arg)
{
  return ob("/"+arg);
}

static string file2(string arg)
{
  return ob("~"+arg);
}

static string sel(string arg)
{
  mixed what;
  string rest;
  object ob,p;
...
static string here(string arg)
{
  push(environment(this_player()));
  return arg;
}

static string env(string arg)
{
  object ob;
  ob=pop();
  if (!err)
...
static string string_desc(string str)
{
  string out;
  out = implode(explode(str,"\\"),"\\\\");
  out = implode(explode(out,"\n"),"\\n");
  out = implode(explode(out,"\""),"\\\"");
  return "\""+out+"\"";
}

static string rec_desc(mixed ob)
{
  if (intp(ob))
    return ""+ob;
  if (stringp(ob))
...
static string array_desc(mixed arr)
{
  string str,line,res;
  string *tmp;
  int i,j;
...
static string clr(string arg)
{
  stack=({});
  return arg;
}

static string dump(string arg)
{
  object ob;
  string s;
...
varargs static string info(string arg)
{
  object ob;
  mixed s;
  int i;
...
static void listinv(object ob, int depth, string prefix)
{
  int i;
  object p;
  if (depth<MAXDEPTH)
...
static string call(string arg)
{
  string func,args;
  int temp,i;
  string rest,tmp,tmp2;
  ...
  while (1)
  {
    ...
    if (sscanf(args,"]%s",rest))
      break;
    if (sscanf(args,"%d.%d%s",tmp,tmp2,rest)==3)
    {
      args=rest;
      argv+=({ to_float(sprintf("%d.%d",tmp,tmp2)) });
      continue;
    }
...
static string result(string arg)
{
  if (objectp(call_result))
    push(call_result);
  else
...
static int destroyable(object ob)
{
  if (!ob)
    return 0;
//  if (object_name(ob)=="/room/void" && 
//      this_player() && environment(this_player())==ob)
//    return 0;
  if (query_once_interactive(ob))
    return 0;
  if (ob==this_object())
...
static void clean(object ob);

static string cln(string arg)
{
  object ob;
  ob=pop();
  if (!err) {
    ...
    {
      write(desc(ob)+" interactive - not cleaned up.\n");
    } else {
      clean(ob);
...
static string dumphists(string arg)
{
  string pl;
  object *ob,master;
  int j,i,hnum;
...
static void cleanof(string str, object ob);

static string clnof(string arg)
{
  object ob;
  int recsave;
  string name,rest;
...
static void Remove(object ob, int a)
{
...
static string db_info(string arg)
{
  object ob;
...
static string inheritlist(string arg)
{
  object ob;
  int i;
  string *inherited;
...
static string make(string arg)
{
  object *list, ob, env, *errs;
  ...
  {
    write("Error moving "+
        implode(map(map(errs,#'getuid),#'capitalize),", ")+
        " out. Aborting.\n");
    for (i=sizeof(list)-1;i>=0;i--)
      if (list[i] && environment(list[i])!=ob)
...
static string rec(string arg)
{
  recursive=1;
  return arg;
}

static string norec(string arg)
{
  recursive=0;
  return arg;
}

static string readvar(string arg)
{
  string rest;
  int no;
...
static string writevar(string arg)
{
  string rest;
  int no;
  object ob;
...
static string align(string s, int x)
{
...
static string stat(string arg)
{
  ...
  if (!(pl=query_once_interactive(ob)))
    level="Monster="+explode(object_name(ob),"#")[0];
  else
    if (IS_GOD(ob))
      level="Mud-Gott";
    // else if (IS_ARCHON(ob))
    //   level="Archon";
    else if (IS_ARCH(ob))
      level="Erzmagier";
    else if (IS_ELDER(ob))
  ...
  if (pl)
    ...
    printf("    Aggressiv : %4s          Gesprächig : %d%%\n",
  ...
  printf(" Lebenspunkte : [%4d/%4d]   Magiepunkte : [%4d/%4d]   " +
	 ...
	 ob->QueryProp(P_XP));
  printf("      Nahrung : [%3d/%d]    Flüssigkeit : [%3d/%d]       " +
	 "Alkohol : [%3d/%d]\n",
  ...
  printf("   Vergiftung : [%3d/%d]   Forscherpunkte : %4d       "
         "Stufenpunkte : %4d\n",
	 ob->QueryProp(P_POISON), ob->QueryProp(P_MAX_POISON),
         ob->QueryProp(P_EP_NUM), ob->QueryProp(P_LP) );
  switch(ob->QueryProp(P_GENDER))  {
    case FEMALE : tmp2 = "weiblich " ; break ;
    case MALE   : tmp2 = "männlich" ; break ;
    default     : tmp2 = "sächlich" ; break ;
  }
  ...
  {
    tmp = explode(break_string(armour[0..sizeof(armour) - 3],
			       63), "\n") ;
    armour = tmp[ 0 ] ;
    tmp=tmp[1..];
...
// 8.Zeile : Geloeste Aufgaben
  if(pl)
  {
    ...
    {
      tmp = explode(break_string(tmp2[0..<3],63), "\n") ;
      tmp2 = tmp[ 0 ] ;
      tmp=tmp[1..];
      for(; sizeof(tmp); tmp=tmp[1..])
        tmp2 += "\n                " + tmp[ 0 ] ;
  ...
  if(environment(ob))
    printf("   Aufenthalt : %s%s.\n",
	   explode(object_name(environment(ob)),"#")[0], tmp2) ;

  return arg;
}

static string scan(string arg)
{
  object ob;
  mixed tmp,tmp2,tmp3;
  int i;
  ...
  printf("Rüstung: %-6d    Waffen: %-6d      Vorsicht: %-6d   Geschlecht: ",
...
static void dump_list(string *what)
{
  int i,s;
...
static string callout(string args)
{
  dump_list(get_callout());
  return args;
}

static string heartbeat(string args)
{
  dump_list(get_heartbeat());
  return args;
}

static string dumplists(string args)
{
  string filen;
  string *list;
  int i,s;
...
static mixed renew_player(string arg)
{
  object ob;
...
static mixed copy(string arg)
{
  ...
  new=clone_object(explode(object_name(ob),"#")[0]);
