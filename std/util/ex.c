// SilberLand MUDlib
//
// ex.c -- ex compatible editor
// $Id: ex.c,v 3.1 1997/03/13 02:39:35 Woody Exp mud $
// $Date: 1997/03/13 02:39:35 $
/* $Revision: 3.1 $
 * $Log: ex.c,v $
 * Revision 3.1  1997/03/13 02:39:35  Woody
 * prepared for revision control
 *
 * Revision 3.0  1995/11/22 14:32:51  Jof
...
int commandMode(string in)
{
  ...
  if(to && to < from) return error("Erste Adresse größer als die Zweite");
  ...
  case 'h':
    write("ex: Kommandohilfe\n\n"
	 ...
	 "  r   -- Datei einfügen\n"
	 "         r<datei>\n"
	 "  a   -- Text hinter der aktuellen Zeile einfügen\n"
	 "  i   -- Text vor der aktuellen Zeile einfügen\n"
	 "  d   -- Zeilen löschen\n"
	 "  y   -- Zeilen zwischenspeichern\n"
	 "  pu  -- Zwischengespeicherte Zeilen einfügen\n"
  ...
  case 'q':
    if(ctrl[buf][CHG])
      if(!(sizeof(cmd) > 1 && cmd[1]=='!'))
        return error("Datei wurde geändert! Abbrechen mit q!");
  ...
  case 's':
  {
    ...
    if(sizeof(scmd = explode(cmd[1..], cmd[1..1])) < 2)
...
varargs int ex(mixed text, closure func, mixed fargs, string buffer)
{
  ...
  if(!text) text = "";
  c = sizeof(text);
  l = sizeof(text = efun::explode(text, "\n")) - 1;
